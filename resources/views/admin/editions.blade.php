        ...
        @foreach($editions as $edition)
            <form action="{{ route('admin.update') }}" method="POST">
                ...
                <div class="flex flex-col rounded p-3 border border-neutral-300 shadow-xs">
                    ...
                    <input name="edition_name" value="{{ $edition->edition_name }}" type="text" id="small-input"
                    ...
                    <textarea name="content_2" id="message" rows="3"
                              class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Leave a comment...">{{ $edition->content_2 }}</textarea>

                    <label for="message" class="block mb-2 text-sm font-medium text-gray-900">Contenu 3</label>
                    <textarea name="content_3" id="message" rows="3"
                              class="block p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"
                              placeholder="Leave a comment...">{{ $edition->content_3 }}</textarea>
