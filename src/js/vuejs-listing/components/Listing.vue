            ...
            <div class="kg-listing-list-map">
                <div class="container-items-links">
                    ...
                    <div class="switch-button">
                        <button type="button" class="btn-liste"
                                v-on:click="activeListe=true,activeMap=false,showMap=false"
                                v-bind:class="classActiveListe">
                            <svg class="icon">
                                <use xlink:href="/assets/images/sprite.svg#liste"></use>
                            </svg>
                            <span>Liste</span></button>
                        <button type="button" class="btn-map" v-on:click="activeMap=true,activeListe=false,showMap=true"
                                v-bind:class="classActiveMaps">
                            <svg class="icon">
...
export default {
    ...
    computed: {
        ...
        getArtisans() {
            this.artisansJson = fetch("http://127.0.0.1:8000/artisansJson").then(res => res.json()).then(data => console.log(data))
        }
    },
    methods: {
        async getData() {
            const res = await fetch("http://127.0.0.1:8000/artisansJson");
